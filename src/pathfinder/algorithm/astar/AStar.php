...
class AStar extends Algorithm {
    ...
    public function start(): Algorithm{
        ...
        $startBlock = $this->getChunkManager()->getBlockAt($startNode->getFloorX(), $startNode->getFloorY(), $startNode->getFloorZ());
    ...
    protected function tick(): void{
        ...
        while($this->checkTimout()) {
            ...
            foreach(self::SIDES as $SIDE) {
                ...
                if($valid && !($checkedSides[$SIDE[0].$SIDE[1]] = $validator->isSafeToStandAt($this, $side))){
                    ...
                    if(!$success) {
                        ...
                        for($y = 0; $y <= $fallDistance; ++$y) {
                            if(!$validator->isSafeToStandAt($this, $side->subtract(0, $y, 0))) {
								continue;
							}
                            $side->y -= $y;
                            $success = true;
				...
				$v = $side->subtract(0, 1, 0);
				$b1 = $manager->getBlockAt($v->getFloorX(), $v->getFloorY(), $v->getFloorZ());
				$b2 = $manager->getBlockAt($side->getFloorX(), $side->getFloorY(), $side->getFloorZ());

                $cost = $costCalculator::getCost($b1) * $costCalculator::getCost($b2);
    ...
    protected function finish(): void{
        ...
        $pathResult = new PathResult( $this->startVector3, $this->targetVector3);
