...
class BossBar
{
	...
	public function sendRemovePacket(array $players): void {
		/** @var Player $player */
		foreach ($players as $player) {
			$pk = RemoveActorPacket::create($this->actorId);
	...
	protected function sendSpawnPacket(array $players): void
	{
		...
		foreach ($players as $player) {
			$pk = AddActorPacket::create(
				...
				$this->actorId,
				EntityIds::PLAYER,
				$player->getLocation()->asVector3()->subtract(0, 5, 0),
				null, 0,0,0, 0,
				array_map(function(Attribute $attr) : NetworkAttribute{
					return new NetworkAttribute($attr->getId(), $attr->getMinValue(), $attr->getMaxValue(), $attr->getValue(), $attr->getDefaultValue(), []);
				}, $this->attributeMap->getAll()), $this->getPropertyManager()->getAll(),
