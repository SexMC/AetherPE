...
class WarpOpenSubCommand extends AetherSubCommand {
	protected function prepare() : void{
		$this->setPermission("skyblock.command.warp.open");
	...
	public function onRun(CommandSender $sender, string $aliasUsed, array $args) : void{
		if($sender instanceof Player){
			Await::f2c(function() use($sender, $args){
				...
				$open = $selected->open === true ? "Opened" : "Closed";
				$sender->sendMessage(Main::PREFIX . $open . " warp §c" . $selected->name);
