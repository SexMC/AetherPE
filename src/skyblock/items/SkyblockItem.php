...
abstract class SkyblockItem extends Item{
	...
	public abstract function buildProperties() : SkyblockItemProperties;
	...
	public function makeUnique() : void{
		$this->getNamedTag()->setString("skyblock_unique_id", uniqid(self::NBT_PREFIX . mt_rand(1, 10000)));
	}

	public function isGlowing() : bool{
		return $this->hasEnchantment(EnchantmentIdMap::getInstance()->fromId(138));
	}

	public function makeGlow() : void{
		$this->addEnchantment(new EnchantmentInstance(EnchantmentIdMap::getInstance()->fromId(138)));
	}
	...
	public function resetLore(array $lore = []) : void{
		...
		$string = "§r{$rarity->getColor()}§l» {$rarity->getDisplayName()}";
