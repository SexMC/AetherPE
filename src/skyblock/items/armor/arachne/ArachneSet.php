...
class ArachneSet extends ArmorSet {
	...
	public function getItemAttributes(string $piece) : array{
		...
		switch($piece) {
			...
			case self::PIECE_CHESTPLATE:
				...
				$arr[] = new ItemAttributeInstance(SkyBlockItemAttributes::HEALTH(), 85);
				break;

			case self::PIECE_LEGGINGS:
				$arr[] = new ItemAttributeInstance(SkyBlockItemAttributes::DEFENSE(), 50);
				$arr[] = new ItemAttributeInstance(SkyBlockItemAttributes::HEALTH(), 75);
				break;
			case self::PIECE_BOOTS:
				$arr[] = new ItemAttributeInstance(SkyBlockItemAttributes::DEFENSE(), 30);
				$arr[] = new ItemAttributeInstance(SkyBlockItemAttributes::HEALTH(), 45);
				break;
		}
	...
	public function onWear(AetherPlayer $player) : void{
		...
		foreach($player->getArmorInventory()->getContents() as $content){
			...
			$player->getPveData()->setDefense($player->getPveData()->getDefense() + $stat);
			$player->getPveData()->setMaxHealth($player->getPveData()->getMaxHealth() + $stat);
		}
	...
	public function getLore(Item $item) : array{
		...
		return [
			...
			"§r§l§3 » §r§a+$stat " . PveUtils::getHealth(),
			"§r§l§3 » §r§a+$stat " . PveUtils::getDefense(),
			"§r",
			"§r§3§lSpecial Ability: Arachne's Faithful ($stack/8)",
			"§r§l§3 » §r§3Stacking multiple armor pieces of this",
