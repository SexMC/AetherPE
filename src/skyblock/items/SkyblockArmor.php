...
use skyblock\items\masks\MaskHolderTrait;
use skyblock\items\masks\MasksHandler;

class SkyblockArmor extends Equipment implements IMaskHolder{
	use MaskHolderTrait;
	...
	public function __construct(ItemIdentifier $identifier, string $name = "Unknown", SkyBlockArmorInfo $armorInfo = null){
		parent::__construct($identifier, $name);

		$this->setProperties(($this->getProperties())->setType(SkyblockItemProperties::ITEM_TYPE_ARMOR)->setRarity($armorInfo->getRarity()));
		...
		if($this->getArmorSet()){
			...
			$this->getNamedTag()->setString("armor_set", $this->getArmorSet()->getIdentifier());
		} else {
			$this->setItemAttribute(new ItemAttributeInstance(SkyBlockItemAttributes::DEFENSE(), $armorInfo->getDefensePoints()));
		}
