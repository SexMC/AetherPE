...
class BudgetHopper extends SkyblockItem implements ItemComponents{
	use ItemComponentsTrait;

	public function __construct(ItemIdentifier $identifier, string $name = "Unknown"){
		parent::__construct($identifier, $name);

		$this->initComponent("budget_hopper", new CreativeInventoryInfo(CreativeInventoryInfo::CATEGORY_EQUIPMENT));

		$this->setCustomName("§r" . $this->properties->getRarity()->getColor() . "Budget Hopper");
		$this->getProperties()->setDescription([
			...
			"§r§7are sold for §a90%§r§7 of their selling",
			"§r§7price",
		]);
