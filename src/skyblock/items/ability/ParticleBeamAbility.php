...
class ParticleBeamAbility extends ItemAbility {
	...
	public function __construct(private Particle $particle, private float $baseDamage, private int $range, private Position $start, private ?Position $target, string $abilityName, int $manaCost, int $cooldown){
	...
	protected function execute(AetherPlayer $player, Item $item) : bool{
		Await::f2c(function() use($player, $item) {
			...
			foreach($this->getLine($player, 1) as $v){
				...
				$bb = new AxisAlignedBB($v->x, $v->y, $v->z, $v->x, $v->y, $v->z);

				foreach($this->start->getWorld()->getNearbyEntities($bb->expandedCopy(2, 2, 2)) as $e){
					if($e instanceof PveEntity){
						if(in_array($e->getId(), $alreadyDamaged)) continue;

						$ev = new PlayerAttackPveEvent($player, $e, $this->baseDamage, false, 0.4);
						$ev->setCause($this);
						$ev->call();
	...
	public function getLine(Player $player, float $addition): array {
		...
		for($d = $addition; $d < $direction->length(); $d += $addition) {
			$locations[] = (clone $this->start)->addVector((clone $direction)->normalize()->multiply($d));
		}
