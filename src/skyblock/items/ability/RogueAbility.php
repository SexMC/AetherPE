...
class RogueAbility extends ItemAbility {
	...
	public function __construct(private int $speedboost, string $abilityName, int $manaCost, int $cooldown){
		parent::__construct($abilityName, $manaCost, $cooldown);
	}

	protected function execute(AetherPlayer $player, Item $item) : bool{
		$player->getPveData()->setSpeed($player->getPveData()->getSpeed() + 100);
