...
class DestroyConnectedBlocksAbility extends ItemAbility {
	...
	public function __construct(private Block $block, private int $destroyCount, string $abilityName, int $manaCost, int $cooldown){
			...
			while($count < $this->destroyCount){
				...
				foreach($last->getAllSides() as $side){
					if($side instanceof $block){
						if(in_array((string) $side->getPosition()->asVector3(), $arr)){
							continue;
						}

						if(!$player->isOnline()) break 2;
						...
						$block->getPosition()->getWorld()->useBreakOn($side->getPosition(), $item, $player, true);
