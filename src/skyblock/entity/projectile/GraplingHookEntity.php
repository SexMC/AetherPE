...
class GraplingHookEntity extends Throwable {
	...
	public function
	__construct(Location $pos, Entity $owner, ?CompoundTag $nbt = null) {
	...
	public function entityBaseTick(int $tickDiff = 1) : bool {
		...
		if($owner instanceof Player) {
			if(!$owner->getInventory()->getItemInHand() instanceof GraplingHook or !$owner->isAlive() or $owner->isClosed()) {
				$this->flagForDespawn();
			}
		} else {
			$this->flagForDespawn();
