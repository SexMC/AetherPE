...
class MinerMinion extends BaseMinion {
	...
	protected function onInitialize() : void{
		...
		if($type = MinionHandler::getInstance()->getMinerType($this->getString("type", ""))){
			$this->type = $type;
		} else {
			$this->flagForDespawn();
			Main::debug("Could not find type for miner minion. Despawning in world {$this->getWorld()->getFolderName()}");
		}
	}

	public function setupMinionLevelInstance() : void{
		$this->level = MinerMinionLevelInitializer::getInstance()->getLevel($this->type->getBlock(), $this->getInt("level"));
	}
	...
	protected function doOfflineCalculations() : void{
		...
		$drops = $this->type->getBlock()->getDrops(VanillaItems::DIAMOND_PICKAXE());
	...
	protected function onTick() : void{
		...
		if(!$this->addItem($random->getDrops(VanillaItems::DIAMOND_PICKAXE()))){
	...
	protected function setupAppearance() : void{
		$this->itemInHand = VanillaItems::WOODEN_PICKAXE();
	...
	public function getEggItem() : Item{
		...
		$item->setLore([
			...
			"§r§7start generating and mining",
			"§r§7{$name}!",
			"§r",
			"§r§7Requires an open",
