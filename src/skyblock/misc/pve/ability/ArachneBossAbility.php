...
class ArachneBossAbility extends MobAbility{
	...
	public function onTick(PveEntity $entity, int $tick) : void{
		...
		if($tick % 10 && mt_rand(1, 100) === 1){
			...
			$e->sendMessage(Main::PREFIX . "§dArachne §2used§d Venom Shot on you hitting you for §a" . number_format($diff) . " damage§d and infecting you with venom.");
			$e->getEffects()->add(AetherEffect::fromPlayer($e, new EffectInstance(VanillaEffects::POISON(), 20 * 5, 10)));
		}
	}
	...
	public function onDeath(PveEntity $entity, EntityDeathEvent $event) : void{
		...
		for($i = 0; $i <= 2; $i++){
			...
			if(isset($keys[$i])){
				...
				if($entity->loottable){
					...
					var_dump("has loottable");
					if($p === null) continue;
					var_dump("gives item");
					var_dump("totaldrops: " . $entity->totaldrops);
		...
		AreaHandler::SPIDERS_DEN()->message($message);
