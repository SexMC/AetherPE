...
abstract class BaseCommand extends Command implements IArgumentable, IRunnable, PluginOwned {
	...
	public function execute(CommandSender $sender, string $usedAlias, array $args) {
		...
		if($passArgs !== null) {
			...
			if($sender instanceof AetherPlayer) {
				$cmd->onPlayerRun($sender, $usedAlias, $passArgs);
			}

			if($sender instanceof ConsoleCommandSender){
				$cmd->onConsoleRun($sender, $usedAlias, $passArgs);
			}
		}
	}
	...
	public function onConsoleRun(ConsoleCommandSender $player, string $aliasUsed, array $args): void {}
